...
use crate::helpers::{apply_odr_bw, apply_sensitivity, apply_axes_enabled, extract_axes_enabled, extract_odr_bw, extract_sensitivity};
use crate::registers::*;
use crate::DriverError;
use crate::errors::map_i2c;
use crate::settings::{Settings, Sensitivity, OutputDataRate, Bandwidth, AxesEnabled, SENSITIVITY_FACTORS};
...
impl<I2cBus: i2c::I2c> GyroscopeDriver<I2cBus> {
    ...
    fn read_register(&mut self, register: u8, buffer: &mut [u8]) -> Result<(), DriverError> {
        self.i2c.write_read(SADR, &[register], buffer).map_err(map_i2c)?;
        Ok(())
    }
    fn write_register(&mut self, register: u8, buffer: &[u8]) -> Result<(), DriverError> {
        ...
        self.i2c.write(SADR, &new_buffer[..total]).map_err(map_i2c)?;
        Ok(())
    }
    ...
    pub fn read_settings(&mut self) -> Result<Settings, DriverError> {
        ...
        Ok(Settings{ sensitivity, data_rate, bandwidth, axes_enabled})
    }
    ...
    pub fn write_odr_bw(&mut self, data_rate: OutputDataRate, bandwidth: Bandwidth) -> Result<(), DriverError> {
        let mut buf = [0u8; 1];
        self.read_register(CTRL_REG1, &mut buf)?;
        buf[0] = apply_odr_bw(buf[0], data_rate, bandwidth);
        self.write_register(CTRL_REG1, &buf)?;
